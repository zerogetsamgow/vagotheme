...
Additional non palette colours `vago.lilac` and  `vago.grey` are included to use as background colours.

```{r lilac,echo=FALSE, out.height="30px",out.width="120px"}
plot(c(vago.lilac,vago.grey))
```
...
```{r col_example_full_colour, warnings = FALSE}
# Create tibble of data for examples
car_accidents = 
  tibble(
...
```{r col_example_single_colour, warnings = FALSE}
...
```{r stack_col_example_full_colour, warnings = FALSE}
...
ggplot(
  ...
  theme_vago_white()
```
```{r stack_col_example_single_colour, warnings = FALSE}
## example of single colour column plot
...
```{r stack_col_example_share, warnings = FALSE}
# Add shares to data
car_accidents =
  car_accidents |> 
  group_by(month) |> 
...
```{r stack_col_example_forecast, warnings = FALSE}
...
```{r bar_stacked_example, warnings = FALSE}
...
```{r single_line_example, warnings = FALSE}
# Line chart data 
line_data = 
  tibble(
    year = 
...
```{r multi_line_example, warnings = FALSE}
## example plot
ggplot(
...
```{r target_line_example, warnings = FALSE}
## example plot
ggplot(
  data = line_data |> filter(str_detect(council, "A")), 
  aes(
...
```{r multi_line_one_colour_example, warnings = FALSE}
## example plot
ggplot(
  data = line_data |> filter(str_detect(council, "A")), 
  aes(
...
```{r multi_line_multi_colour_example, warnings = FALSE}
## example plot
ggplot(
